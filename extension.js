...
function parseOpenMeteo(data,aqData,pollen,unit) {
    ...
    return {
        ...
        hourly,daily,
        allergens:{
            treeRisk:pollen?.treeRisk??null,grassRisk:pollen?.grassRisk??null,weedRisk:pollen?.weedRisk??null,
            treeCount:pollen?.treeCount??null,grassCount:pollen?.grassCount??null,weedCount:pollen?.weedCount??null,
            pm25:aq('pm2_5'),pm10:aq('pm10'),
        },
    };
}
...
async function fetchAmbeePollen(lat,lon,key) {
    ...
    try {
        const d=await fetchAmbeeJSON(
            `https://api.ambeedata.com/latest/pollen/by-lat-lng?lat=${lat}&lng=${lon}`,key);
        const entry=d?.data?.[0];
        if(!entry){console.warn('[WeatherPrime] Ambee pollen: unexpected response',JSON.stringify(d));return null;}
        return {
            treeRisk:entry.Risk?.tree_pollen??null,grassRisk:entry.Risk?.grass_pollen??null,weedRisk:entry.Risk?.weed_pollen??null,
            treeCount:entry.Count?.tree_pollen??null,grassCount:entry.Count?.grass_pollen??null,weedCount:entry.Count?.weed_pollen??null,
        };
    } catch(e){console.error('[WeatherPrime] Ambee pollen error:',e.message);return null;}
}
...
async function fetchAlerts(lat,lon) {
    try {
        ...
        return (data.features??[]).map(f=>({
            event:f.properties.event??'Alert',headline:f.properties.headline??'',
            desc:f.properties.description??'',severity:f.properties.severity??'Unknown',
        }));
    } catch{return[];}
}
...
class WeatherPanel {
    ...
    _build() {
        ...
        [['current','Now'],['hourly','Hourly'],['daily','7-Day'],['allergens','🌿 Allergens']].forEach(([id,lbl])=>{
            ...
            this._tabBtns[id]=btn;tabBar.add_child(btn);
        });
    ...
    setError(msg){
        ...
        box.add_child(label('⚠️','wp-error-icon'));box.add_child(label(msg,'wp-error-msg'));
    ...
    _renderAlertsBanner(alerts) {
        ...
        alerts.forEach(a=>{
            ...
            tr.add_child(eventLbl);item.add_child(tr);
            if(a.headline){const hl=label(a.headline,'wp-alert-headline');hl.clutter_text.line_wrap=true;item.add_child(hl);}
    ...
    _renderCurrent() {
        ...
        right.add_child(label(c.temp,'wp-cur-temp'));right.add_child(label(c.desc,'wp-cur-desc'));
        ...
        [['Feels like',c.feelsLike],['Humidity',c.humidity],['Wind',c.wind],['Pressure',c.pressure]].forEach(([k,v])=>{
            ...
            cell.add_child(label(k,'wp-detail-key'));cell.add_child(label(v,'wp-detail-val'));
            grid.add_child(cell);
        });
    ...
    _renderHourly() {
        ...
        this._data.hourly.forEach(h=>{
            ...
            row.add_child(label(h.time,'wp-hour-time'));row.add_child(label(h.icon,'wp-hour-icon'));
            row.add_child(spacer());row.add_child(label(h.temp,'wp-hour-temp'));
            box.add_child(row);
        });
        this._content.add_child(box);
    }
    _renderDaily() {
        ...
        this._data.daily.forEach(d=>{
            ...
            row.add_child(label(d.day,'wp-day-name'));row.add_child(label(d.icon,'wp-day-icon'));
            row.add_child(spacer());row.add_child(label(d.hi,'wp-day-hi'));
            row.add_child(label(' / ','wp-day-sep'));row.add_child(label(d.lo,'wp-day-lo'));
    ...
    _renderAllergens() {
        ...
        [['Tree',a.treeRisk,a.treeCount],['Grass',a.grassRisk,a.grassCount],['Weed',a.weedRisk,a.weedCount]].forEach(([name,risk,count])=>{
            const lvl=pollenRisk(risk),row=hbox('wp-allergen-row');
            row.add_child(label(name,'wp-allergen-name'));row.add_child(spacer());
            const valStr=count!=null?`${Math.round(count)} gr/m³ — ${lvl.text}`:lvl.text;
            const lbl=label(valStr,'wp-allergen-val');
...
class WeatherIndicator extends PanelMenu.Button {
    ...
    async _fetch(force=false) {
        ...
        const intervalMs=Math.max(5,this._settings.get_int('fetch-interval'))*60*1000;
        if(!force&&this._cachedParsed&&(Date.now()-this._lastFetch)<intervalMs){
            if(this._locName)this._panel.setLocation(this._locName);
            this._panel.setData(this._cachedParsed);return;
        }
        ...
        try {
            ...
            const [aqData,pollen,alerts]=await Promise.all([
                ...
                fetchAmbeePollen(this._lat,this._lon,ambeeKey),
